...
DISPLAY_ROWS = int(240 * 2)
DISPLAY_COLS = int(ratio * DISPLAY_ROWS)
print(f"c{DISPLAY_COLS},{COLS} - r{DISPLAY_ROWS},{ROWS}")
...
# for file in files: # ["col0.bytes"]: # files:
    ...
    with open(fullPath, "rb") as imageBytes:
        ...
        print(f"Read {file} length: {len(arr)}")
        ...
        print(f"Rmap shape=", rmap.shape, "Rmap avg=", np.mean(rmap), "Rmap sum=", np.sum(rmap), end="\t")
        ...
        if np.sum(rmap) > magnitude_threshold:
            ...
            if computeX:
                ...
                # print(f"Iterating 0..{DISPLAY_COLS}")
            ...
            if drawRectangle:
                ...
                while r not in explored and r > 0 and r < DISPLAY_ROWS:   # assumes concave distribution of p across argument r
                    ...
                    tolerance = 0.6
                    p = radialPercent(rmap, meanX, meanY, r) - (np.pi / 4) * tolerance
                    explored[r] = p
                    # print(f"rad {r} -> {p}")
                    if p > 0:
                        r+=1
