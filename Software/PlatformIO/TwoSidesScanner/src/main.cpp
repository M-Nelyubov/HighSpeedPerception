...
// Split L and R halves of screen and broadcasting net flow for estimate of frame to frame motion
...
#define SSD_STEP_SIZE 2       // how many pixels to move over between each SSD attempt.  
...
uint8_t corners[IMAGE_HEIGHT][IMAGE_WIDTH];

double integrated_L = 0;
double integrated_R = 0;

bool t = 0;   // timestep of latest image to go into buffer
#define THRESHOLD 255/150 //integer threshold for corner pixel - at least a 150 out of 255, but inverted for faster compute

#define MAX_PIX_CORR 256   // MAXIMUM_PIXEL_CORRESPONDENCES - the upper bound on how many corresponding points to try to match between two subsequent frames
...
void cornerDetect(int t) {
    ...
    for (int row = 0; row < IMAGE_HEIGHT - corner_sl; row++) {
        for (int col = 0; col < IMAGE_WIDTH - corner_sl; col++) {
            ...
            // end the iteration through the pixel grid early if the number of matches reaches the maximum
            if(ent == MAX_PIX_CORR) {
                row = IMAGE_HEIGHT;
                col = IMAGE_HEIGHT;
                break;
...
void computeUV(){
    ...
    corr_en[0] = 0;  // "clear" all corners by setting the first one as disabled
    cornerDetect(t);
    ...
    // int net_U = 0;// net motion = ul+ur
    int cl = 0;   // count of left-half points
    int cr = 0;   // count of right-half points
    int ul = 0;   // sum of u-values that shift left
    int ur = 0;   // sum of u-values that shift right

    // partition this across L vs R halves of the image

    // Serial.printf("X,Y,U,V tuples: ");
    for(uint8_t i = 0; i < currentFrameCorners; i++) {
      int8_t px = corr_X[i];   // pixel x value
      int8_t py = corr_Y[i];   // pixel y value
      int8_t pu = corr_U[i];   // pixel u value (L-R shift)
      if (px > IMAGE_WIDTH/2){ // right half of the image
        cr++;
        ur += pu;
      } else {  // left half of the image pixel space
        cl++;
        ul += pu;
      }
    }
    ...
    float fcl = cl;
    float fcr = cr;
    float mul = ful/fcl;  // mu_l - average shift left
    float mur = fur/fcr;  //        average shift right

    integrated_L += mul;
    integrated_R += mur;

    Serial.printf("t:%000000d  ", times[t]);
    Serial.printf("L:% 000.1f  ", integrated_L);
    Serial.printf("R:% 000.1f  ", integrated_R);
    Serial.println("");
    // Serial.printf("C(L): % 0d.   Σ(L): % 00d.   μ(L): % 00.2f.   I(L): % 000.1f", cl, ul, mul, integrated_L);
    // Serial.printf("C(R): % 0d.   Σ(R): % 00d.   μ(R): % 00.2f.   I(R): % 000.1f", cr, ur, mur, integrated_R);
    // Serial.printf("C(A): % 0d.   Σ(A): % 00d.   μ(A): % 00.2f.   ", cr+cl, ur+ul, (ful+fur) / (fcl+fcr));
    // Serial.println("");
